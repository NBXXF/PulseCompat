...
protocol ConsoleEntitiesSource {
    ...
    final class ConsoleSearchViewModel: ObservableObject, ConsoleSearchOperationDelegate {
        ...
        init(environment: ConsoleEnvironment, source: ConsoleEntitiesSource, searchBar: ConsoleSearchBarViewModel) {
            ...
            recents = ConsoleSearchRecentSearchesStore(mode: environment.mode)
            self.source = source
            self.searchBar = searchBar

            context = store.newBackgroundContext()
            scopes = Set(allScopes)
        ...
        private func configure(mode _: ConsoleMode) {
            suggestionsViewModel = getRecentSearches()
        ...
        private func updateSearchTokens() {
            ...
            if searchText.isEmpty {
                ...
                suggestionsViewModel = ConsoleSearchSuggestionsViewModel(searches: [])
            }
        }

        private func getRecentSearches() -> ConsoleSearchSuggestionsViewModel {
            let recentSearches = recents.searches.prefix(3).map { term in
                ConsoleSearchSuggestion(text: AttributedString("\(term.options.title) ") { $0.foregroundColor = .primary } +
                    AttributedString(term.text) { $0.foregroundColor = .accentColor }, action: .apply(term))
            }
